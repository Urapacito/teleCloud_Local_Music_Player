...
const SignalPathModal = ({ currentFile, selectedDevice, devices, eqEnabled, setEqEnabled, onClose }) => {
  ...
  const ext = currentFile?.path?.split('.').pop()?.toUpperCase() || 'UNKNOWN';
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', left: '35px', top: '40px', bottom: '40px', width: '2px', background: 'var(--bg-tertiary)' }} />
        ...
        <div style={{ display: 'flex', gap: '15px', position: 'relative', zIndex: 1 }}>
          ...
          <div>
            <div style={{ fontWeight: 'bold', fontSize: '14px' }}>Source</div>
            <div style={{ fontSize: '12px', color: 'var(--accent-red)' }}>{currentFile?.metadata?.title || 'Unknown Audio'}</div>
